...
def _compute_reorder_schedule(current_stock, daily_demand, lead_time_days, moq, safety_weeks):
    """
    Simulate forward to produce a list of reorder events.

    Logic:
    1. Walk day-by-day, tracking inventory.
    2. At each day, check: "If I don't order today, will I stock out
       before the next order could arrive (lead_time_days from now)?"
    3. If yes, place an order. The order arrives lead_time_days later.
    4. Order quantity = max(MOQ, demand during lead_time + safety stock period
       rounded up to nearest MOQ multiple).

    Returns list of dicts:
        order_date, arrival_date, quantity, inventory_at_order,
        projected_inventory_at_arrival, days_until_stockout
    """
    if not daily_demand:
        return []

    safety_days = safety_weeks * 7
    horizon_days = len(daily_demand)

    # Build a quick lookup: day_index → demand
    day_demands = [d for _, d in daily_demand]
    day_dates = [d for d, _ in daily_demand]

    # Track inventory over time
    inventory = float(current_stock)
    orders = []  # list of (arrival_day_index, quantity)
    schedule = []

    i = 0
    while i < horizon_days:
        # Add any arriving orders
        for arr_idx, qty in orders:
            if arr_idx == i:
                inventory += qty

        # Consume today's demand
        inventory -= day_demands[i]

        # Look ahead: will inventory cover the next lead_time + safety period?
        lookahead_end = min(i + lead_time_days + safety_days, horizon_days)
        lookahead_demand = sum(day_demands[i + 1:lookahead_end])

        # Also account for orders already in transit
        inbound = sum(qty for arr_idx, qty in orders if i < arr_idx <= lookahead_end)

        projected = inventory + inbound - lookahead_demand

        if projected <= 0 and inventory >= -day_demands[i]:  # time to order
            # Calculate how much we need for the next order cycle
            # Cover lead_time + safety + some runway beyond arrival
            cover_end = min(i + lead_time_days + safety_days + lead_time_days, horizon_days)
            cover_demand = sum(day_demands[i + 1:cover_end])

            # What we'll have when the order arrives
            arrival_idx = i + lead_time_days
            demand_until_arrival = sum(day_demands[i + 1:min(arrival_idx, horizon_days)])
            inv_at_arrival = inventory - demand_until_arrival + inbound

            # Need enough to cover from arrival through next reorder cycle
            post_arrival_end = min(arrival_idx + lead_time_days + safety_days, horizon_days)
            post_arrival_demand = sum(day_demands[min(arrival_idx, horizon_days):post_arrival_end])

            needed = post_arrival_demand - max(inv_at_arrival, 0)
            needed = max(needed, 0)
            ...
            if needed <= 0:
                ...
                order_qty = math.ceil(needed / moq) * moq

            arrival_day = min(i + lead_time_days, horizon_days - 1)
            orders.append((arrival_day, order_qty))

            # Projected inventory when order arrives
            proj_at_arrival = inv_at_arrival + order_qty

            # Days until stockout from today (without this order)
            temp_inv = inventory
            days_to_stockout = 0
            for j in range(i + 1, horizon_days):
                temp_inv -= day_demands[j]
                temp_inv += sum(qty for arr_j, qty in orders[:-1] if arr_j == j)
                if temp_inv <= 0:
                    days_to_stockout = j - i
                    break
            else:
                days_to_stockout = horizon_days - i

            schedule.append({
                "order_date": day_dates[i],
                "arrival_date": day_dates[arrival_day] if arrival_day < horizon_days else day_dates[-1] + timedelta(days=lead_time_days - (horizon_days - 1 - i)),
                "quantity": order_qty,
                "inventory_at_order": round(inventory),
                "projected_inventory_at_arrival": round(proj_at_arrival),

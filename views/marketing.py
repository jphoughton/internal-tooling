...
from analytics.daily_metrics import load_shopify_daily, load_amazon_daily, load_gs_spend


@st.cache_data(ttl=300)
def _load_shopify_daily_metrics():
    """Load daily DTC metrics — thin wrapper with Streamlit cache."""
    return load_shopify_daily()


@st.cache_data(ttl=600)
def _load_gs_spend():
    """Load ad spend from Google Sheet — thin wrapper with Streamlit cache."""
    return load_gs_spend()


@st.cache_data(ttl=600)
def _load_amazon_daily():
    """Load Amazon daily data — thin wrapper with Streamlit cache."""
    return load_amazon_daily()
    ...
    if _has_shopify_data:
        ...
        if not mkt_df.empty:
            ...
            if _has_goals:
                ...
                _cm_amz_repeat_rev = max(_cm_amz_rev - min(_cm_amz_nc_rev, _cm_amz_rev), 0)
                _l7d_amz_repeat_rev = max(_l7d_amz_rev - min(_l7d_amz_nc_rev, _l7d_amz_rev), 0)
                _yd_amz_repeat_rev = max(_yd_amz_rev - min(_yd_amz_nc_rev, _yd_amz_rev), 0)
                ...
                if _nav_channel == 'Rollup':
                    ...
                    _rollup_rows.extend([
                        ...
                        build_pace_row("Repeat Revenue", _cm_total_repeat_rev, _goal_repeat_rev,
                                       _l7d_ret_rev + _l7d_amz_repeat_rev, _yd_ret_rev + _yd_amz_repeat_rev,
                                       _pct_month, _days_in_month, _remaining_days),
                    ])
                ...
                if _nav_channel in ('Rollup', 'Amazon'):
                    ...
                    _amz_rows.append(
                        build_pace_row("New Customer Rev", _cm_amz_nc_rev, 0, _l7d_amz_nc_rev, _yd_amz_nc_rev,
                                       _pct_month, _days_in_month, _remaining_days),
                    )
                    _amz_rows.append(
                        build_pace_row("Repeat Rev", _cm_amz_repeat_rev, 0, _l7d_amz_repeat_rev, _yd_amz_repeat_rev,
                                       _pct_month, _days_in_month, _remaining_days),
                    )

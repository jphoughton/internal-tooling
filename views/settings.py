...
def render(ctx):
    ...
    st.caption(
        "Daily breakdown of new vs repeat customers and revenue by channel. "
        "Uses MIN(order_date) for first-order classification (matches Shopify admin)."
    )

    try:
        with get_db() as conn:
            # DTC (Shopify) — new/repeat customers and revenue by day
            dtc_df = read_sql("""
                WITH cust_first AS (
                ...
                SELECT
                    LEFT(o.order_date, 10) AS day,
                    COUNT(DISTINCT CASE WHEN LEFT(cf.first_order_date, 10) = LEFT(o.order_date, 10)
                                        THEN o.customer_id END) AS dtc_new_customers,
                    COUNT(DISTINCT CASE WHEN LEFT(cf.first_order_date, 10) < LEFT(o.order_date, 10)
                                        THEN o.customer_id END) AS dtc_repeat_customers,
                    SUM(CASE WHEN LEFT(cf.first_order_date, 10) = LEFT(o.order_date, 10)
            ...
            # Amazon — new/repeat customers and revenue by day
            amz_df = read_sql("""
                WITH cust_first AS (
                    SELECT customer_id, MIN(order_date) AS first_order_date
                    FROM orders WHERE source = ?
        ...
        qa = dtc_df.copy() if not dtc_df.empty else pd.DataFrame(columns=['day'])
        if not amz_df.empty:
            qa = qa.merge(amz_df, on='day', how='outer')
        else:
            for c in ['amz_new_customers', 'amz_repeat_customers', 'amz_new_rev', 'amz_repeat_rev']:
                qa[c] = np.nan

        qa = qa.sort_values('day', ascending=False).head(90).reset_index(drop=True)

...
def render(ctx):
    ...
    if summary_df.empty or summary_df['total_units'].sum() == 0:
        ...
        render_html_table(disp_summary, freeze_cols=1)
    ...
    if not rollup_table.empty:
        ...
        render_html_table(ru_display, max_height=min(len(ru_display) * 35 + 38, 700), freeze_cols=3)
    ...
    if not new_customer_table.empty:
        ...
        render_html_table(nc_display, max_height=min(len(nc_display) * 35 + 38, 600), freeze_cols=3)
    ...
    if not repeat_customer_table.empty:
        ...
        render_html_table(rc_display, max_height=min(len(rc_display) * 35 + 38, 600), freeze_cols=3)
    ...
    if 'amazon' in active_sources or (not amazon_table.empty):
        ...
        if not amazon_table.empty:
            ...
            render_html_table(amz_display, max_height=min(len(amz_display) * 35 + 38, 600), freeze_cols=3)
        else:

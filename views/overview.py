...
def _load_overview_stats(date_start=None, date_end=None, source_filter=None):
    date_clause = ''
    date_clause_orders = ''
    params_d = []
    ...
    with get_db() as conn:
        # orders table is Shopify-only; for Amazon source_filter we skip orders queries
        if source_filter == 'amazon':
            total_orders = 0
            total_customers = 0
        else:
        ...
        for r in conn.execute(
            f'SELECT source, SUM(revenue) as rev FROM daily_sku_sales WHERE 1=1 {date_clause}{source_clause} GROUP BY source',
            params_d,
        ).fetchall():
        ...
        source_split = read_sql(
            f'SELECT source, SUM(order_count) as orders, SUM(revenue) as revenue FROM daily_sku_sales WHERE 1=1 {date_clause}{source_clause} GROUP BY source',
            conn, params=params_d,
        )

        # Top SKUs — only available for Shopify (order_items table)
        if source_filter == 'amazon':
            top_skus = read_sql(f"""
                SELECT d.sku, sm.product_name, sm.category,
                       SUM(d.units_sold) as total_units,
                       SUM(d.revenue) as total_revenue
                FROM daily_sku_sales d
                JOIN sku_master sm ON d.sku = sm.sku
                WHERE d.source = 'amazon' {date_clause.replace('sale_date', 'd.sale_date')}
                GROUP BY d.sku, sm.product_name, sm.category
...
def render(ctx):
    ...
    _title_text = f'{channel} Overview' if channel != 'Rollup' else 'Overview'
    ...
    stats = _load_overview_stats(date_start=str(ov_start), date_end=str(ov_end), source_filter=source_filter)

    # -- KPI row --
    if source_filter == 'amazon':
        _k1, _k2, _k3 = st.columns(3)
        _k1.metric('Amazon Revenue', f"${stats['amazon_revenue']:,.0f}")
        _k2.metric('Total Revenue', f"${stats['total_revenue']:,.0f}")
        _k3.metric('Active SKUs', f"{stats['total_skus']:,}")
    elif source_filter == 'shopify':
        _k1, _k2, _k3, _k4 = st.columns(4)
        _k1.metric('Shopify Revenue', f"${stats['shopify_revenue']:,.0f}")
        _k2.metric('Orders', f"{stats['total_orders']:,}")
        _k3.metric('Customers', f"{stats['total_customers']:,}")
        _k4.metric('Total Revenue', f"${stats['total_revenue']:,.0f}")
    else:
    ...
    if source_filter:
        # Single-channel view — show only the filtered source
        nr_single = get_projected_new_repeat_summary(str(ov_start), str(ov_end), source_filter)
        if nr_single.get('gap_days', 0) > 0:
            st.caption(f"*Projected values include DOW-adjusted estimates for missing days. Data through {nr_single['last_data_date']} ({nr_single['gap_days']}d gap).*")

        _nr1, _nr2, _nr3, _nr4 = st.columns(4)
        _nr1.metric('New Customers', _fmt_proj(nr_single['new_customers'], nr_single.get('projected_new_customers', 0)),
                    delta=f"+{nr_single['projected_new_customers']} projected" if nr_single.get('projected_new_customers', 0) > 0 else None)
        _nr2.metric('Repeat Customers', _fmt_proj(nr_single['repeat_customers'], nr_single.get('projected_repeat_customers', 0)),
                    delta=f"+{nr_single['projected_repeat_customers']} projected" if nr_single.get('projected_repeat_customers', 0) > 0 else None)
        _nr3.metric('New Customer Revenue', _fmt_proj(nr_single['new_revenue'], nr_single.get('projected_new_revenue', 0), prefix='$'))
        _nr4.metric('Repeat Customer Revenue', _fmt_proj(nr_single['repeat_revenue'], nr_single.get('projected_repeat_revenue', 0), prefix='$'))
        # Use nr_single as nr_all for the pie chart below
        nr_all = nr_single
    else:
        # Rollup — show combined + per-channel breakdown
    ...
    if source_filter:
        # Single-channel: full-width revenue trend, no pie chart
        ...
        if not daily.empty:
            ...
            _src_color = '#0F3557' if source_filter == 'shopify' else '#F58B3D'
            fig_rev = go.Figure()
            fig_rev.add_trace(go.Scatter(
                x=daily_total['sale_date'], y=daily_total['revenue_7d'],
                mode='lines', name=f'{source_filter.title()} (7d avg)',
        ...
        # Rollup: stacked revenue chart + channel mix pie
    ...
    with nr_col_left:
        ...
        nr_daily = get_new_repeat_daily_revenue(str(ov_start), str(ov_end), source_filter=source_filter)
    ...
    if source_filter == 'shopify':
        # DTC: show only 3PL stock
        _iv1, _iv2, _iv3 = st.columns(3)
        _iv1.metric('3PL Stock', f'{_ov_3pl_total:,.0f}' if _ov_3pl_ok else '\u2014')
        _iv2.metric('Demand (3mo)', f'{_ov_demand_3mo:,.0f}')
        _iv3.metric('Demand (12mo)', f'{_ov_demand_12mo:,.0f}')
        if _ov_3pl_ok and _ov_demand_3mo > 0:
            _monthly_demand = _ov_demand_3mo / 3
            _months_supply = _ov_3pl_total / _monthly_demand if _monthly_demand > 0 else 0
            _supply_color = '\U0001f7e2' if _months_supply >= 3 else ('\U0001f7e1' if _months_supply >= 1.5 else '\U0001f534')
            st.caption(f'{_supply_color} {_months_supply:.1f} months of 3PL supply')
    elif source_filter == 'amazon':
        # Amazon: show only FBA stock
        _iv1, _iv2, _iv3 = st.columns(3)
        _iv1.metric('FBA Stock', f'{_ov_fba_total:,.0f}' if _ov_fba_ok else '\u2014')
        _iv2.metric('Demand (3mo)', f'{_ov_demand_3mo:,.0f}')
        _iv3.metric('Demand (12mo)', f'{_ov_demand_12mo:,.0f}')
        if _ov_fba_ok and _ov_demand_3mo > 0:
            _monthly_demand = _ov_demand_3mo / 3
            _months_supply = _ov_fba_total / _monthly_demand if _monthly_demand > 0 else 0
            _supply_color = '\U0001f7e2' if _months_supply >= 3 else ('\U0001f7e1' if _months_supply >= 1.5 else '\U0001f534')
            st.caption(f'{_supply_color} {_months_supply:.1f} months of FBA supply')
    else:
        # Rollup: show both

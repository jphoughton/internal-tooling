"""Shared pacing dashboard block — current-month revenue vs goal tables.

Extracted so it can be embedded in multiple pages (e.g. Overview and Marketing).
"""
import streamlit as st
import pandas as pd
from db import get_db, read_sql, get_media_spend, get_amazon_revenue_forecast
...
def _clean_num(val):
    """Parse a numeric value from a formatted string (e.g. '$1,234', '12.5%')."""
    if pd.isna(val):
        return 0.0
...
def render_pacing_block(ctx, key_prefix='mkt'):
    """Render the current-month pacing dashboard block.

    Loads Google Sheet data, computes MTD actuals and revenue goals, then renders
    pacing tables for Roll Up, DTC, and Amazon channels.

    Args:
        ctx: shared context dict from the router.
        key_prefix: unique string prefix for Streamlit widget keys — use different
            values when embedding on multiple pages to avoid key conflicts.

    Returns:
        dict with keys:
            'has_data': bool — True if Google Sheet data was available
            'chan_sel': str — current channel filter ('All', 'Roll Up', 'DTC', 'Amazon')
            'mkt_df': DataFrame — date-filtered Google Sheet data (columns pre-computed)
            'mkt_start': date | None
            'mkt_end': date | None
    """
    ...
    # Check if google_sheet_data table exists with a date column
    with get_db() as conn:
        try:
    ...
    mkt_start, mkt_end = smart_date_filter(
        mkt_df['_date'].min().date(), mkt_df['_date'].max().date(), key_prefix,
        default_preset='MTD',
    )
    mkt_df = mkt_df[(mkt_df['_date'].dt.date >= mkt_start) & (mkt_df['_date'].dt.date <= mkt_end)]

    # Compute all metrics.
    # NOTE: Triple Whale "blended" metrics use dual-attribution that reports ~2x
    # actual platform spend & Shopify revenue. We halve affected columns.
    ...
    try:
        with get_db() as _yd_conn:
            ...
            _yd_rollup = _yd_conn.execute(
                'SELECT spend, new_customers, new_customer_rev FROM amazon_daily_rollup WHERE date = ?',
                (_yesterday,),
            ).fetchone()
            if _yd_rollup:
    ...
    if _has_goals:
        def _pace_color(pct, invert=False):
            """Return CSS for pacing cells. invert=True for spend (lower = better)."""
            if invert:
                if pct <= 0.95:
        ...
        if _chan_sel in ('All', 'Roll Up'):
            ...
            eff1.metric('Business MER', f'{_business_mer:.2f}x', help='Total Revenue / Total Spend')
            eff2.metric('Total NC ROAS', f'{_total_nc_roas:.2f}x', help='Total NC Revenue / Total Spend')
            eff3.metric('New Customers', f'{_cm_total_nc:,}', help='DTC new customer orders')
            eff4.metric('Blended CPA', f'${_blended_cpa:,.0f}', help='Total Spend / Total New Customers')
        ...
        if _chan_sel in ('All', 'Amazon'):
            ...
            _amz_rows.append(_build_pace_row('Revenue', _cm_amz_rev, _goal_amz_rev, _l7d_amz_rev, _yd_amz_rev))
            _amz_rows.append(_build_pace_row('NC Revenue', _cm_amz_nc_rev, 0, _l7d_amz_nc_rev, _yd_amz_nc_rev))

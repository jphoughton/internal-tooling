...
def run_parallel_backfill(shopify_years=4, amazon_months=6,
    ...
    logger.info(
        'PARALLEL BACKFILL — Amazon: %s -> %s (%d chunks), '
        'Shopify: %s -> %s (%d chunks), Amazon workers: %d, Shopify workers: %d',
    ...
    summary_lines = [f'BACKFILL COMPLETE in {elapsed}']
    for source, total in sorted(totals.items()):
        summary_lines.append(f'  {source}: {total:,} records')
    if errors:

...
def run_daily_sync(full_refresh=False, on_status=None):
    ...
    if has_shopify:
        ...
        logger.info('Shopify not configured — skipping.')
    ...
    if _klaviyo_key:
        ...
        logger.info('Klaviyo not configured — skipping.')
    ...
    try:
        ...
        if _gs_id:
            ...
            logger.info('Google Sheet not configured — skipping.')
    except Exception as e:
...
def _sync_klaviyo(api_key):
    ...
    if not conversion_metric_id:
        ...
        if conversion_metric_id:
            ...
            if conversion_metric_id:
                logger.info('Using fallback metric for engagement stats: %s', conversion_metric_id)
    ...
    try:
        ...
        logger.warning('Klaviyo metrics fetch failed (metadata still saved): %s', e)
...
def run_parallel_backfill(shopify_years=4, amazon_months=6,
    ...
    logger.info('PARALLEL BACKFILL')
    logger.info('  Amazon:  %s → %s (%d chunks)', amazon_start, end_date, len(amazon_chunks))
    logger.info('  Shopify: %s → %s (%d chunks)', shopify_start, end_date, len(shopify_chunks))
    logger.info('  Amazon workers: %d, Shopify workers: %d', amazon_workers, shopify_workers)
